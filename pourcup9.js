...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Line {
        ...
        angle() {
            this.object = new Point(this.x1, this.y1)
            this.target = new Point(this.x2, this.y2)
    ...
    class SpringOP {
        ...
        balance() {
            ...
            dropoff = (Math.max(1,  Math.min((((Math.abs(this.anchor.xmom) + Math.abs(this.anchor.ymom) + Math.abs(this.body.xmom) + Math.abs(this.body.ymom))))  * (((Math.abs(this.anchor.xmom) + Math.abs(this.anchor.ymom) + Math.abs(this.body.xmom) + Math.abs(this.body.ymom))))  * 1.1 , 20) ))/101
            if(((Math.abs(this.anchor.xmom) + Math.abs(this.anchor.ymom) + Math.abs(this.body.xmom) + Math.abs(this.body.ymom))*.8) < 15){
                dropoff = 0
            }
    ...
    class Water {
        ...
        move(){
            this.body.ymom += (Math.cos(position) * .45)*(Math.max((7-this.links.length),.1)*.5)
            this.body.xmom += (Math.sin(position) * .45)*(Math.max((7-this.links.length),.1)*.5)
            this.body.frictiveMove()
            this.body.repel()
            this.body.color = `rgb(${this.r},${this.g},${this.b})`
            this.body.radius = (this.links.length * 1) + 4
            this.bigbody = new Circle(this.body.x, this.body.y, 14, "#00AAFF80", 0, 1, .99)
            this.bigbody.radius = (this.links.length * 2) + 4
        }
        draw() {
            this.body.draw()
        }
    ...
    class Cup {
        constructor() {
            this.center = new Circle(350, 350, 270, "red", 7, 5,1, 1)
            // this.points = []
            // this.beams = []
        ...
        draw() {
            ...
            for (let t = 0; t < this.drops.length; t++) {
                ...
                while (this.shape.doesPerimeterTouch(this.drops[t].body)) {
                    ...
                    this.drops[t].body.xmom *= .91
                    this.drops[t].body.ymom *= .91
                ...
                for (let k = 0; k < this.drops.length; k++) {
                    if (t != k) {
                        ...
                        if (this.drops[t].body.doesPerimeterTouch(this.drops[k].body)) {
                            this.drops[t].body.xmom -= (this.drops[k].body.x - this.drops[t].body.x) / 30
                            this.drops[t].body.ymom -= (this.drops[k].body.y - this.drops[t].body.y) / 30
                            ...
                            this.drops[t].body.xmom += (Math.cos(angleRadians) * distance) / 2
                            this.drops[t].body.ymom += (Math.sin(angleRadians) * distance) / 2
                            this.drops[k].body.xrepel -= (Math.cos(angleRadians) * distance) / 4.41
                            this.drops[k].body.yrepel -= (Math.sin(angleRadians) * distance) / 4.41
                            this.drops[k].body.xmom -= (Math.cos(angleRadians) * distance) / 2
                            this.drops[k].body.ymom -= (Math.sin(angleRadians) * distance) / 2
                            this.drops[t].body.repel()
                            this.drops[k].body.repel()
                            this.drops[t].r = ((this.drops[t].r*300)+(this.drops[k].r))/301
                            this.drops[t].g = ((this.drops[t].g*300)+(this.drops[k].g))/301
                            this.drops[t].b = ((this.drops[t].b*300)+(this.drops[k].b))/301
                            this.drops[k].r = ((this.drops[k].r*300)+(this.drops[t].r))/301
                            this.drops[k].g = ((this.drops[k].g*300)+(this.drops[t].g))/301
                            this.drops[k].b = ((this.drops[k].b*300)+(this.drops[t].b))/301
                        }
                                
                            if (!this.drops[t].links.includes(k) && !this.drops[k].links.includes(t) && this.drops[t].links.length < 6 && this.drops[k].links.length < 6) {
                                let link = new SpringOP(this.drops[t].body, this.drops[k].body, 30, `rgba(${((this.drops[t].r)+(this.drops[k].r))/2}, ${((this.drops[t].g)+(this.drops[k].g))/2}, ${((this.drops[t].b)+(this.drops[k].b))/2}, .5)`)
